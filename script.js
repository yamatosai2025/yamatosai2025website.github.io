...
const boothData = {
    classes: [
        ...
        { id: 'c23', category: 'class', name: { ja: '2年3組', en: 'Class 2-3' }, content: { ja: 'クラブ 童夢', en: 'Club Domu' }, description: { ja: 'Ladies and gentlemen, boys and girls! 夏の暑さと山東祭の熱気にあてられたそこのアナタ、涼しいクラブで遊んでいきませんか？占いで素敵な未来への恒等式をお教えしましょう。', en: 'Feeling the heat of the summer and the festival? Come cool down and play at our club. We\'ll tell your fortune for a wonderful future.' }, location: { ja: '2-3教室', en: 'Room 2-3' } },
...
window.updateCrowdStatusUI = function(statuses) {
    ...
    if (Object.keys(lastReceivedStatuses).length === 0) {
         grid.innerHTML = `<p style="grid-column: 1 / -1; text-align: center; color: #9ca3af;" data-lang-key="crowdLoading">${translations[currentLang].crowdLoading}</p>`;
         return;
    }
...
createAndDisplayBooths(function applyInitialFilter() {
    ...
    const classButton = document.querySelector('#filters button[data-filter="class"]');
    const allButton = document.querySelector('#filters button[data-filter="all"]');
    
    if (classButton && allButton) {
        // 一瞬だけ「クラス」フィルターを適用
        classButton.click();
        // すぐに「すべて」フィルターに戻す
